...
public void openGmailDirect(String to, String subject, String body) {
    ...
    private JPanel createLoginPage() {
        ...
        Runnable doLogin = () -> {
            try {
                ...
                // Clear fields after submitting (successful or not)
                try { uField.setText(""); } catch (Exception ignored) {}
                try { eField.setText(""); } catch (Exception ignored) {}
                try { pField.setText(""); } catch (Exception ignored) {}
            }
        };
    ...
    private void syncUserPasswordsFromDB() {
        ...
        try {
            ...
            int updated = 0;
            int dbCount = dbUsers.size();
            int inMemoryCount = usersByEmail.size();
            System.out.println("syncUserPasswordsFromDB: dbUsers=" + dbCount + " inMemoryUsers=" + inMemoryCount);
            java.util.List<String> foundWithPasswords = new java.util.ArrayList<>();
            java.util.List<String> updatedEmails = new java.util.ArrayList<>();
            for (DatabaseManager.User du : dbUsers) {
                if (du == null || du.email == null) continue;
                if (du.password != null && !du.password.isEmpty()) foundWithPasswords.add(du.email + "(pwd)");
                User local = usersByEmail.get(du.email.toLowerCase());
                if (local != null && du.password != null && !du.password.isEmpty()) {
                    // update if local missing or different
                    if (local.password == null || local.password.isEmpty() || !du.password.equals(local.password)) {
                        local.password = du.password;
                        updated++;
                        updatedEmails.add(du.email);
                    }
                }
            }
            System.out.println("syncUserPasswordsFromDB: updated passwords for " + updated + " users.");
            if (!updatedEmails.isEmpty()) System.out.println("Passwords updated for: " + String.join(", ", updatedEmails));
            if (!foundWithPasswords.isEmpty()) System.out.println("DB users with passwords: " + String.join(", ", foundWithPasswords));
        } catch (Exception ex) {
