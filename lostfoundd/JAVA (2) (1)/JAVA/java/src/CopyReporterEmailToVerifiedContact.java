...
public class CopyReporterEmailToVerifiedContact {
    public static void main(String[] args) throws Exception {
        if (args.length < 5) {
            System.err.println("Usage: CopyReporterEmailToVerifiedContact <host> <port> <db> <user> <pass>");
            System.exit(2);
        }
        String host = args[0];
        String port = args[1];
        ...
        try (Connection conn = DriverManager.getConnection(url, user, pass)) {
            String upd = "UPDATE verified_reports vr JOIN listings l ON vr.original_listing_id = l.id SET vr.contact_info = l.reporter_email WHERE vr.contact_info IS NULL";
            ...
            try (Statement st = conn.createStatement()) {
                try (ResultSet rs = st.executeQuery("SELECT id, original_listing_id, reporter_email, contact_info FROM verified_reports LIMIT 20")) {
                    System.out.println("--- verified_reports after copy ---");
                    while (rs.next()) {
