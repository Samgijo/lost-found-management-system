...
public class DatabaseManager {
    ...
    public DatabaseManager(String host, int port, String dbName, String user, String password) {
        ...
        try {
            ...
            String url = String.format("jdbc:mysql://%s:%d/%s?serverTimezone=UTC&useSSL=false", host, port, dbName);
            try {
    ...
    private boolean createDatabaseIfMissing(String host, int port, String dbName, String user, String password) {
        String serverUrl = String.format("jdbc:mysql://%s:%d/?serverTimezone=UTC&useSSL=false", host, port);
                ...
                if (rs.next()) {
                    ...
                    for (String d : desired) {
                        ...
                        switch (d) {
                            ...
                            case "status": values.add(status); break;
                    ...
                    // If verified_reports uses original_listing_id as a foreign key to listings,
                    // deleting the parent row will violate the FK. In that case, update the
                    // listing status to 'Verified' instead of deleting. Otherwise, delete.
                    if (vrCols.contains("original_listing_id")) {
                        try {
    ...
    public void moveVerifiedListingsToReports() throws SQLException {
        if (conn == null) return;
    String sql = "SELECT id FROM listings WHERE lower(status) = 'verified'";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            try (ResultSet rs = ps.executeQuery()) {
